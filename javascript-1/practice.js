...
let middleNums = numbers.slice(2, 5);
...
myNumbers.forEach(function(val,index,myNumbersIndex){
	return arr[index]
	myNumbersIndex.push(arr[index]);
});
...
let enemies = people.filter(function(element,index){
    if (index[element]===false){
...
let totallyAwesome = people.reduce(function(acc,a,b){
  return acc + b[a];
})